...
Every `user_dossier` or `user_override` claim, in any mode, carries a `quote`: at least four characters, or the whole input, copied from the content of a typed request input of that same type. Validation compares it after NFC normalization with runs of whitespace folded to one space, and drops a space only beside Chinese or Japanese script, so a quote wrapped across YAML lines still matches while "is notable" does not stand for "is not able"; and fails with `AUTHORING_USER_CLAIM_QUOTE_REQUIRED` when it is missing or `AUTHORING_USER_CLAIM_QUOTE_UNBOUND` when no such input contains it. Write the `statement` so it says what the quote says; a reviewer reads the two side by side. More `user_override` claims than override inputs returns the advisory `AUTHORING_USER_OVERRIDE_CLAIMS_EXCEED_INPUTS`.

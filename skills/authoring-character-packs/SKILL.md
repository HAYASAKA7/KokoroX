...
Compile only with `character draft compile`, only after valid deterministic results, and only into the trusted configured data root. Never run pack compilation, installation, public publication, session activation, relationship-state, or event mutation commands—even under deadline or authority pressure.

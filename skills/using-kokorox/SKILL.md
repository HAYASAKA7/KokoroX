...
4. Resolve or compile the language policy, then run `kokorox runtime plan` to build the typed render plan. Pass `--context` with the saved context from step 2 so the pack's own authored lines can enter the plan; without it the character never speaks its own words. Pass artifact paths beneath `KOKOROX_DATA_DIR`; preserve every protected channel and immutable span.
